import os
from sqlalchemy import Column, create_engine, Boolean, BigInteger, ForeignKey, Integer, String
...
class Group(Base):
    ...
    users = relationship("ChatUser", back_populates="group", cascade="all, delete")
    settings = relationship("GroupSettings", back_populates="group", uselist=False, cascade="all, delete-orphan")
    badwords = relationship("BadWord", back_populates="group", cascade="all, delete-orphan")
...
class GroupSettings(Base):
    ...
    def __repr__(self):
        return f"<GroupSettings(group_id={self.group_id}, slow_mode={self.slow_mode})>"
